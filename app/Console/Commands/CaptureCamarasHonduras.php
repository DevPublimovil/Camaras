...
class CaptureCamarasHonduras extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'capture:honduras';
     ...
     * @var string
     */
    protected $description = 'Command para capturar camaras de honduras';
        ...
        $ch = curl_init();
            for ($i=0; $i < 60; $i++)
            {
                foreach ($pantallas as $key => $pantalla)
                {
                        ...
                        $carpeta=  ($pantalla->country_id == 4) ? 'honduras/'.\Carbon\Carbon::now()->locale('es')->isoFormat('dddd').'/'.str_replace([' ','.',','],'_',substr($pantalla->name,9)) : 'nicaragua/' .\Carbon\Carbon::now()->locale('es')->isoFormat('dddd').'/'.str_replace([' ','.',','],'_',substr($pantalla->name,9));
