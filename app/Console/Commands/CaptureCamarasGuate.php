...
class CaptureCamarasGuate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'capture:guate';
     ...
     * @var string
     */
    protected $description = 'Command para capturar camaras de guatemala';
        ...
        $ch = curl_init();
            for ($i=0; $i < 80; $i++)
            {
                foreach ($pantallas as $key => $pantalla)
                {
                        ...
                        $carpeta=  'guatemala/'.\Carbon\Carbon::now()->locale('es')->isoFormat('dddd').'/'.str_replace([' ','.',','],'_',substr($pantalla->name,9));
